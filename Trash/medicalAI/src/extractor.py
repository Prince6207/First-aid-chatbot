...
class ExtractedSymptom:
    """Represents a symptom extracted from text."""
    text: str                          # Raw extracted text span
    confidence: float                  # Model confidence [0, 1]
    start: int                         # Start char position
    end: int                           # End char position
    entity_type: str = "SYMPTOM"       # Entity label
...
class BioBERTExtractor:
    ...
    SYMPTOM_ENTITY_TYPES = {
        "SIGN_SYMPTOM", "SYMPTOM", "DISEASE_DISORDER",
        "BIOLOGICAL_ATTRIBUTE", "CLINICAL_EVENT"
    }
    ...
    def __init__(self, model_name: str = "d4data/biomedical-ner-all",
                 device: str = "cpu", use_fallback_if_error: bool = True):
    ...
    def extract(self, text: str) -> ExtractionResult:
        ...
        Args:
            text: Patient's symptom description in natural language.

        Returns:
            ExtractionResult with list of ExtractedSymptom objects.
        """
    ...
    def _extract_with_model(self, text: str) -> ExtractionResult:
        ...
        try:
            ...
            for ent in entities:
                ...
                if any(sym_type in label.upper() for sym_type in
                       ["SIGN", "SYMPTOM", "DISEASE", "BIOLOGICAL"]):
        ...
        for pattern in self.SYMPTOM_KEYWORDS:
            for m in re.finditer(pattern, text_lower, re.IGNORECASE):
                ...
                if span not in seen_spans:
                    seen_spans.add(span)
                    result.symptoms.append(ExtractedSymptom(
...
def extract_symptoms(text: str, model_name: str = "d4data/biomedical-ner-all") -> ExtractionResult:
    """
